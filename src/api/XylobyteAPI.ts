...
const host = 'https://api.xylobyte.com';

export class XylobyteAPI {
	static getHardSkills = async (): Promise<HardSkills> => await fetchData('/v1/website/hardskills');

	static getSoftSkills = async (): Promise<SoftSkill[]> => await fetchData('/v1/website/softskills');

	static getLanguagesSkills = async (): Promise<LanguageSkill[]> => await fetchData('/v1/website/languages-skills');
...
const fetchData = async (url: string) => {
	const res = await axios.get(host + url, {
		adapter: import.meta.env.SSR ? 'http' : undefined,
	});
